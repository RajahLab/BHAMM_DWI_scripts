...
#
# Inputs: 
# (1) SPSS file of the BHAMM Master database (Last updated: October 2023).
...
master <- read_sav("input-file-name.sav") %>% clean_names() # n = 468
...
session_two <- read_sav("input-file-name.sav") %>% 
  clean_names() %>%
  select(c("id", "dw1_scan_num")) # n = 261
...
bhamm <- filter(bhamm, s2_age >= 39.5 & s2_age < 65.5) # n = 188
...
bhamm <- bhamm %>% filter((sex == -1 & enrol_gender == 1) | 
                          ...
                          (sex == 1 & is.na(enrol_gender))) # n = 187
...
bhamm <- filter(bhamm, !dw1_scan_num %in% c("NA", NA)) # n = 128

# Filter out participants scanned with old DWI parameters (checked separately)
bhamm <- filter(bhamm, !id %in% c(0001)) # n = 117
...
cooks_d <- data.frame(id = bhamm$id, 
                      ...
                      miss_hard_d = cooks.distance(lm_miss_hard)) 
